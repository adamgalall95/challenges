...
export const createRecipeString = (ingredientsArr) => {
  let recipe = ingredientsArr[0];
  for (let i = 1; i < ingredientsArr.length; i++) {
    recipe = recipe + "+" + ingredientsArr[i];
  }
  return recipe;
};
...
export const generateAverage = (numberArr) => {
  if (numberArr.length === 0) {
    ...
    let numberElem = numberArr.length;

    let sumArr = totalScores(numberArr);

    let avg = Math.round(sumArr / numberElem);
...
export const reverseOrder = (toReverseArr) => {
  let newArr = [];
  let lastIndex = toReverseArr.length - 1;
  for (let i = 0; i < toReverseArr.length; i++) {
    newArr[lastIndex] = toReverseArr[i];
    --lastIndex;
  }
  return newArr;
};
