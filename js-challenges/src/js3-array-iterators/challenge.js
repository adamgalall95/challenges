...
export const removeFalseValues = (booleanArr) => {
  const newArr = booleanArr.filter((val) => val == true);
  return newArr;
};
...
export const createPercentageList = (numbersArr) => {
  const percen = (num) => `${num * 100}%`;
  const newArr = numbersArr.map(percen);
  return newArr;
};
...
export const createListOfPoessessions = (possessionsArr, name) => {
  const newArr = possessionsArr.map((val) => name + " " + val);
  return newArr;
};
...
export const convertStringToNumbersArray = (numberString) => {
  const numbList = numberString.split("+");
  const newArr = numbList.map((val) => Number(val));
  return newArr;
};
...
export const createOddEvenArray = (numberString) => {
  ...
  const newArr = numbList.map(evenOdd);
  return newArr;
};
...
export const filterBooksBySearch = (booksArr, searchTerm) => {
  // const findTerm = (text, term) => text.includes(term);
  // const newArr = booksArr.filter((val) => findTerm(val, searchTerm));
  const newArr = booksArr.filter((val) => val.includes(searchTerm));
  return newArr;
};
