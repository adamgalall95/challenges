...
export const totalScoresArr = (scoresArr) => {
  const total = scoresArr.reduce((sum, num) => sum + num, 0);
  return total;
};
...
export const reverseString = (toReverse) => {
  ...
  const revArr = stringArr.map((val) => val.split("").reverse().join(""));
...
export const sortCharactersAlphabetically = (charcterArr) => {
  const sortAlpa = charcterArr.map((value) => value.toLowerCase()).sort();
  return sortAlpa;
};
